...
module.exports = {
	 ...
	 * @type {string | string[]} */
	ownerId: process.env.DEVUID || ["880055530721247253"],
	/** 
	 * Token for bot login
	 * @type {string} */
	...
	nodes: [
		{
			identifier: "Node-1", // log id string
			host: "54.36.225.156",
			port: 2333,
			password: "s4DarqP$&y",
	...
	*/
	permissions: 277083450689, // 8 = Administrator, 0 = Doesn't need permissions (uses slash commands)
	...
	*/
	...
	presence: {
		...
		activities: [
			{
				name: "{someVariable} Dream Land | Pond of Dream",
				type: "LISTENING",
				data: (client) => {
					return {
            ...
            {
                name: "{orangDongo} Dream Wellers | Pond of Dream",
                type: "LISTENING",
                data: (client) => {
                    return {
                        orangDongo: client.guilds.cache.reduce((a, b) => a + b.memberCount, 0),
                    }
                }
            }
		],
